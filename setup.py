...
def setup_app(app):
    ...
    def log_request():
        ...
        app.config['request_logs'].insert(0, {
    ...
    @app.route('/dashboard')
    def dashboard():
        total_requests = len(app.config['request_logs'])
        unique_ips = len(set(log['ip'] for log in app.config['request_logs']))
        endpoint_counts = {}
        for log in app.config['request_logs']:
            endpoint_counts[log['endpoint']] = endpoint_counts.get(log['endpoint'], 0) + 1
        uptime = datetime.datetime.now() - app.config['start_time']
        days = uptime.days
        seconds = uptime.seconds
        hours = seconds // 3600
        minutes = (seconds % 3600) // 60
        seconds = seconds % 60
        return render_template('dashboard.html', total_requests=total_requests, unique_ips=unique_ips, endpoint_counts=endpoint_counts, logs=app.config['request_logs'], domain=request.host_url.rstrip('/'), days=days, hours=hours, minutes=minutes, seconds=seconds)
