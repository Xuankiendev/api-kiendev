...
def dashboard():
    ...
    seconds = seconds % 60
    return render_template('dashboard.html', total_requests=total_requests, unique_ips=unique_ips, endpoint_counts=endpoint_counts, logs=request_logs, domain=request.host_url.rstrip('/'), days=days, hours=hours, minutes=minutes, seconds=seconds, start_time=start_time.timestamp() * 1000)
