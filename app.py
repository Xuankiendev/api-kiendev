...
def get_lyrics(song_id):
    try:
        ...
        return {"err": -1, "msg": "Không thể lấy lời bài hát"}

def validate_api_key(api_key):
    if not api_key:
        return {"error": "Thiếu khóa API", "status_code": 401}
    if api_key not in VALID_API_KEYS:
        return {"error": "Khóa API không hợp lệ", "status_code": 401}
...
def bancheck():
    ...
    if not api_key:
        return Response(json.dumps({"error": "Thiếu ApiKey", "status_code": 401}), mimetype="application/json")
